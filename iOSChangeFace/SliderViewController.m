...
- (void)viewDidLoad
{
    ...
    [self showContentControllerWithModel:_MainVC!=nil?NSStringFromClass([_MainVC class]):@"FTF_RootViewController"];
...
- (void)closeSideBar
{
    ...
    if ([self.target respondsToSelector:self.closeAction]) {
        SuppressPerformSelectorLeakWarning(
        [self.target performSelector:self.closeAction withObject:nil];
                                           );
    }
...
- (void)configureViewShadowWithDirection:(RMoveDirection)direction
{
    ...
    _mainContentView.layer.shadowOpacity = 0.24f;
}
